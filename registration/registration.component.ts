...
export class RegistrationComponent implements AfterViewInit {
  ...
  saveData(): void {
    ...
    this.initialDataSource.push({ name, phone });
  ...
  applySearch(event: Event) {
    ...
    if (searchValue) 
      this.dataSource = this.dataSource.filter(value => value.name.startsWith(searchValue.trim().toLowerCase()) || value.phone.startsWith(searchValue.trim().toLowerCase())) ;
  }
